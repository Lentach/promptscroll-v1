...
export function PromptGrid({
        ...
        <div className="bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-xl rounded-3xl p-8 max-w-md mx-auto border border-white/10 relative z-10">
          ...
          <div className="mt-4 space-y-2">
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {['business', 'creative', 'coding', 'marketing'].map((suggestion) => (
                <button
                  key={suggestion}
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="flex items-center space-x-2 bg-white/10 rounded-full px-3 py-1">
                {sortBy === 'newest' ? (
                  <>
                    ...
                    <span className="text-blue-300 font-medium">Newest first</span>
                  </>
                ) : sortBy === 'popular' ? (
                  <>
                    <Flame className="h-4 w-4 text-orange-400 animate-pulse" />
                    <span className="text-orange-300 font-medium">Most popular first</span>
                  </>
                ) : (
                  <>
                    <TrendingUp className="h-4 w-4 text-green-400 animate-pulse" />
                    <span className="text-green-300 font-medium">Trending (14 days)</span>
                  </>
                )}
              </div>
      ...
      {/* Enhanced Prompts Grid with staggered animations */}
        ...
        {prompts.map((prompt, index) => (
          <div
            ...
            className={`
              ...
              ${highlightedPromptId === prompt.id 
                ? 'ring-2 ring-blue-500 ring-opacity-50 bg-blue-500/5 rounded-3xl scale-105 shadow-2xl shadow-blue-500/20' 
                : ''
              }
            `}
            style={{
          ...
          >
            <PromptCard 
              prompt={prompt} 
              isTopPrompt={sortBy === 'popular' && index < 10}
