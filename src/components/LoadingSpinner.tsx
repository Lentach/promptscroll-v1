  ...
  if (variant === 'gradient') {
    return (
      <div className={`relative ${sizeClasses[size]} ${className}`}>
        ...
        <div className={`absolute inset-0 rounded-full border-2 ${variantClasses.gradient} animate-spin`} />
  ...
  if (variant === 'pulse') {
    return (
      <div className={`${sizeClasses[size]} ${className} relative`}>
        {/* Main spinner */}
        <div className={`animate-spin rounded-full border-2 ${variantClasses.pulse} ${sizeClasses[size]}`} />
        
        {/* Pulsing background */}
        <div className={`absolute inset-0 rounded-full border-2 border-blue-500/30 animate-ping ${sizeClasses[size]}`} />
      </div>
    )
  }

  return (
    <div className={`animate-spin rounded-full border-2 ${variantClasses.default} ${sizeClasses[size]} ${className}`} />
  )
}