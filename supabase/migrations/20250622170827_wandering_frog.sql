-- Migration: Enhanced PromptScroll Features with Comments, Sessions, and Rate Limiting
-- This migration adds comprehensive features while avoiding policy conflicts
...
-- FIXED: Nowa polityka dla anonimowych użytkowników - pozwala na approved dla demo
CREATE POLICY "Allow anon insert for demo prompts"
  ON prompts
  FOR INSERT
  TO anon
...
-- 15. POLITYKI RLS DLA KOMENTARZY (bezpieczne tworzenie)
ALTER TABLE comments ENABLE ROW LEVEL SECURITY;

-- Bezpieczne usuwanie istniejących polityk
DO $$
BEGIN
...
-- 16. POLITYKI RLS DLA USER_SESSIONS (bezpieczne tworzenie)
ALTER TABLE user_sessions ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  DROP POLICY IF EXISTS "Allow public read own session" ON user_sessions;
  DROP POLICY IF EXISTS "Allow public insert session" ON user_sessions;
  DROP POLICY IF EXISTS "Allow public update own session" ON user_sessions;
EXCEPTION
  WHEN undefined_object THEN
    NULL;
END $$;
...
-- 17. POLITYKI RLS DLA POPULAR_TAGS (bezpieczne tworzenie)
ALTER TABLE popular_tags ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  DROP POLICY IF EXISTS "Allow public read popular tags" ON popular_tags;
EXCEPTION
  WHEN undefined_object THEN
    NULL;
END $$;
...
BEGIN
  ...
  VALUES (
    ...
    ARRAY[primary_model_param], 'approved', 0, 0, 0, 0.0, false, false
  )
  ...
  RETURN QUERY SELECT true, 'Prompt added successfully.', new_prompt_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
